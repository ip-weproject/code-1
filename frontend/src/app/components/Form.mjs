  ...
  return (
    <section className="text-p-gray-700 flex lg:flex-row flex-col items-stretch">
      <div className="lg:w-1/2 lg:px-24 py-6 px-2 flex flex-col lg:items-start lg:text-left items-center text-center">
        <h1 className="title-font sm:text-4xl text-3xl mb-4 font-medium text-color-cian">
          Let's talk
        </h1>
        <p className="mb-8 leading-relaxed">
          Subscribe to our newsletter to get the latest stories of our work and
          many happy customer testimonials. Want to be our customer? Enter your
          email below.
        </p>
        <form
          ...
          method="POST"
          data-netlify="true"
          className="flex flex-col lg:justify-start justify-center w-full max-w-md p-2"
        >
          {/* Campo oculto requerido para que Netlify identifique el formulario */}
          <input type="hidden" name="form-name" value="contact" />
          <textarea
            name="message"
            className="focus:outline-none shadow-lg border-2 border-gray-300 mb-4 py-2 px-3"
            rows="4"
            placeholder="Your message here..."
          ></textarea>
          <div className="flex flex-row justify-between shadow-lg border-2 border-gray-300">
            <input
              className="flex-grow bg-white focus:outline-none px-4 m-0"
              placeholder="Email"
              name="email"
              type="email"
              required
            ...
            <button
              type="submit"
              className="inline-flex text-p-black font-semibold py-2 px-6 focus:outline-none text-md m-0 h-12 bg-p-gray-300"
            >
              Send Message
            </button>
          </div>
        </form>
        <div className="text-sm mt-2 text-p-gray-500 mb-8 w-full">
