...
function Start() {
  ...
  return (
    <section className="bg-color-2 w-full h-full md:px-32 px-10 flex flex-col md:flex-row justify-center">
    {/* <div className="font-albert-bold w-full min-h-screen flex justify-center relative z-0 bg-[url('/images/portada1.jpg')] bg-cover bg-center bg-no-repeat after:content-[''] after:absolute after:inset-0 after:bg-black/60 after:z-[-1]"> */}
      ...
      <div className="w-1/2 max-w-6xl mx-auto flex flex-col mt-10 md:mt-16">
    ...
    if (domNode.name === "highlight") {
      return <span className="special-highlight">{domNode.children[0].data}</span>;
        ...
        <div className="w-full flex justify-center mt-4 md:mt-4">
          <span className="font-gilroy-light text-color-1 md:text-2xl px-24 md:px-0 text-start mt-0 md:mt-0">
            {t("home_text_1")}
          </span>
        </div>
        {/* Botón de consulta */}
        <div className="flex flex-col md:items-start w-auto items-center md:text-start md:mt-10 mt-2 md:mb-12 mb-4">
        <a className="consultation-btn mt-4 md:mt-10 px-6 py-4 rounded-full text-lg font-gilroy-regular text-color-1 hover:consultation-btn transition-all duration-300"
        onClick={() => window.open("https://calendly.com/ip-weproject")}>
        {t("book_consultation")}
      </a>
        </div>
      </div>
      <div className="mt-0 md:mt-6 flex justify-center md:justify-end">
        <Image
          priority
          width={500}
          height={500}
